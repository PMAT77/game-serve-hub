...
- **实例详情页新增「文件管理」**：可以浏览实例目录、直接编辑文本配置（房间、世界、Mod 等 `.ini` / `.lua` / `.json` / `.txt` / `.cfg` / `.log`），并支持重命名与删除。保存前自动备份原文件（同目录生成带时间戳的 `.bak`），单个文本文件读写上限 1 MiB（超出只显示前 1 MiB 并提示）；集群令牌等敏感文件标注「敏感」，不提供查看、编辑、重命名与删除。文件操作全部限制在实例目录内：绝对路径、`..` 与指向目录外的符号链接都会被拒绝。**上传与下载仍不在面板内提供**，搬运文件请用「备份与恢复」或 SFTP。
