...
- **「面板端口」不再被本地启动悄悄改掉，设置页也不再自相矛盾**：此前只要在本机跑一遍生产配置，这个设置项就会被改写成后端监听端口（8888），于是设置页出现「输入框写着 8888、旁边却提示当前访问端口是 9527」的画面——两个数字都"对"，指的却不是同一件事，用户既看不懂也不知道该信哪个。根因是端口发布变量带了一个兜底值，被当成「用户显式声明的对外端口」参与启动校正；现在未显式声明时不再回写数据库，已经设置过的值也不会被动。设置页同时改成按部署形态说人话：服务器上显示「面板端口」，本地开发时显示「前端开发服务器端口」（它真正影响的就是本地前端页面端口）；「当前实际端口」只在保存的端口与正在用的端口不一致时才出现，一致时不再多写一行，且取值改由服务端判定（兼容反向代理与端口转发），不再读浏览器地址栏推算。**本地已经被改成 8888 的面板端口，在设置页改回 9527 保存一次即可。**

- **「面板端口」改完是真的会换端口了**：此前这个设置只写数据库，重启面板也不会改变实际端口（面板实际端口只由部署配置决定），因此「需重启面板才能生效」一直是句空话。现在保存时会把新端口写进部署配置——Docker 部署借一键更新同款的一次性容器写入 `panel.env` 的 `PANEL_PORT`（先备份，刻意不重建面板），Native 部署直接写 `SERVER_PORT`；用户下次重启面板即生效，保存后界面会提醒放行新端口。部署目录不可达、配置文件只读等无法自动写入的情况，会给出可直接复制执行的命令，而不是默默失败。
