...
| **存档备份** | 某个实例整个世界的打包文件，留在服务器上、进「备份与恢复」的列表，用来**回档**。来源有手动、定时，以及系统在**更新前**、**删除前**、**恢复前**、**导入前**、**回档前**、**重置世界前**自动创建的安全备份 |
| **迁移包** | 实例详情页「导出迁移包」产出的包，给**另一台机器**导入用，附带迁移报告（分片端口、Mod 清单、风险项）。它不进备份列表、导出 24 小时后自动清理，也不能用来回档——与存档备份不是一回事 |
| **面板数据库快照** | 对面板自身 SQLite 的一致性备份（账号、实例记录与面板设置），**不含任何游戏存档**；可下载、删除、导入外部快照，也可在面板内恢复——恢复会替换面板数据并让它重启 |
| **计划任务** | 面板的定时器：定时备份、定时重启、更新检查、面板数据库快照 |
