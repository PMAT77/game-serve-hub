...
# v0.9.0 统一镜像：三键同值（占位 registry 待 resolve_image_registry 替换）
[[ "${GSH_RELEASE_TAG}" == "v0.9.0" ]]
[[ "${PANEL_IMAGE}" == "" ]]
[[ "${GSH_GAME_DST_IMAGE}" == "" ]]
[[ "${GSH_STEAMCMD_IMAGE}" == "" ]]
...
finalize_image_refs
[[ "${PANEL_IMAGE}" == "ghcr.io/pmat77/game-server-hub:v0.9.0" ]]
