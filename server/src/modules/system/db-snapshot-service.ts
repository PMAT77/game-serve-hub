 ...
 * 面板数据库快照的创建与保留策略。
 *
 * 从 `db-backup-routes.ts` 拆出来：恢复流程（`db-restore-service.ts`）在替换数据库之前
 * 也要先给当前库留一份快照，两处必须用同一个实现——各自写一份的话，某一处的快照
 * 迟早会漏掉保留策略或落到别的目录。
 */
...
/** 快照文件名时间戳（YYYYMMDD-HHMMSS），与实例备份保持同一种可读格式 */
...
/** 淘汰超出保留上限的最旧数据库快照 */
export async function enforceDbSnapshotRetention(app: FastifyInstance, retention: number): Promise<void> {
  if (retention <= 0) {
    return
  }
...
export interface CreateDatabaseSnapshotOptions {
  /** 备注；缺省为「面板数据库一致性快照」 */
  note?: string
}
 ...
 * 手动路由、计划任务 db_snapshot 与「恢复面板数据前的自动快照」共用；retention 沿用备份设置。
 */
export async function createDatabaseSnapshot(
  app: FastifyInstance,
  createdBy: string,
  options: CreateDatabaseSnapshotOptions = {},
  ...
  const record = await createBackupRecord({
    ...
    filePath: targetPath,
    sizeBytes,
    note: options.note?.trim() || '面板数据库一致性快照',
    kind: 'database',
    status: 'completed',
    createdBy,
  })
