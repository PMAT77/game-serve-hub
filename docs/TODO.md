...
| SteamCMD 与游戏安装     | 🔶 部分完成 | 仅饥荒 DST（AppID `343050`）；v1 进程直启；安装日志文件持久化 + 手动更新 API；Docker 游戏容器延后 M2 |
| 游戏市场 / 多游戏         | ❌ 未开始   | 无独立市场页；可安装列表仅 1 款                                                       |
| 游戏实例控制台            | 🔶 部分完成 | 日志 SSE + 命令下发已完成；**命令补全 / 快捷基础命令**待做（见 §5） |
...
- [x] 实例列表维度的 **CPU/内存占用、运行时长**（`POST /app/instance/metrics` + `runtime_started_at`；列表列展示，运行中 5s 轮询指标）。
...
- [~] SteamCMD 执行器：
  ...
  - [x] 游戏服务端**手动更新**专用 API/入口（`POST /app/instance/update` + 实例列表「更新服务端」）。
...
- [x] 游戏实例实时控制台日志（SSE `/app/instance/console/stream` + 轮询补齐）。
- [x] 向游戏服务端发送命令（`POST /app/instance/console/command`，stdin 管道）。
- [x] 控制台基础交互：自动滚动、清空、复制（`node/instance/console.vue`）。
- [x] 服务重启后校正 DB 与真实进程不一致的运行状态（`reconcileStaleRunningInstances`）。

**待做（命令体验增强，控制台层保持游戏无关，命令表按 `gameCode` 扩展）**

- [ ] **命令输入补全**：输入框支持历史记录（↑/↓）、本地缓存最近 N 条；按当前实例 `gameCode` 加载候选命令列表（前缀匹配 / 下拉建议）。
- [ ] **基础命令快捷发送**：后端维护「游戏 → 预设命令」配置（显示名、实际下发文本、运行前确认级别）；前端快捷按钮区一键发送（无需手敲 Lua）。
- [ ] **DST 首批快捷命令**（对齐常见面板能力，映射 stdin 控制台）：
  - 保存存档（如 `c_save()`）
  - 回档 1～6 天（如 `c_rollback(n)`）
  - 重置世界（如 `c_reset()`，需二次确认）
- [ ] **可选增强**：日志下载为文件；分片/世界切换（DST Master/Caves 多进程时再拆）。
...
- 输入过滤与命令注入防护基线（实例路径已有基础校验，需覆盖**控制台命令下发**与快捷命令白名单场景）。
...
- [~] **M1（第 1-4 周）**：登录 + 节点/实例基础管理 + 系统监控台 + **游戏实例控制台 v1** + **实例列表资源指标**已具备；**命令补全/快捷命令**仍缺。
...
- [x] `installPercent`、`installLogStatus` 与文件持久化安装日志（`data/install-logs/`）；`lastCommand` 仍为进度/操作摘要。
- [ ] 安装字段 `installPhase` 独立语义（当前由 `lastCommand` 文案推断阶段）。
...
### 游戏实例控制台（`node/instance/console.vue`）

- [ ] **命令补全与快捷基础命令**：见 §5「待做」；优先 DST 保存/回档/重置，接口保持通用（`gameCode` 驱动命令表）。
...
2. [x] **游戏实例控制台（v1）**：`console` 模块（日志流 + 命令下发），与监控台路由/命名区分清晰。
3. [ ] **控制台命令补全与快捷命令**：输入历史/补全 + 按 `gameCode` 预设命令与快捷按钮（DST 保存、回档、重置优先）。
4. [x] **安装链路补齐**：安装日志持久化、手动更新 API；v1 明确进程模式并更新 `DEVELOPMENT.md` 验收口径（Docker 游戏容器延后 M2）。
5. [x] **实例列表可观测性**：单实例 CPU/内存、运行时长采集与展示。
6. [ ] **对外安装入口文档**：README 中补充 `curl`/`wget` 与 Release 版本化脚本地址。
