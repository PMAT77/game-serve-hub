...
B 站系列视频按「先能装通、再能用好」的顺序录制，只演示[安装与运维](docs/INSTALL.md)里已有的路线，命令以文档为准。

目前计划三集，正在制作中：

| 集 | 主题 |
| --- | --- |
| V1 | 这是什么面板：功能、适合谁、现状、优势与不足 |
| V2 | Native 模式一键安装：不装 Docker |
| V3 | Docker 模式一键安装：含 GHCR 拉不动的解法 |

发布后这里会替换为合集链接。在此之前，安装与排错请直接看[安装指南](docs/INSTALL.md)，开服操作看 [DST 开服教程](docs/DST_TUTORIAL.md)。

## 快速开始

当前为 `v0.6.15` 公测线。要求 Ubuntu 22.04 / 24.04 或 Debian 12，root/sudo，至少 4 GiB 内存和 4 GiB 空闲磁盘（离线镜像包约 227 MB，导入后本地镜像约 560 MB；游戏本体与存档另需数 GB）；Native 正式支持 x86_64，Docker 的 ARM64 支持仍为实验性。
...
```bash
curl -fsSL https://cdn.jsdelivr.net/gh/PMAT77/game-serve-hub@v0.6.15/scripts/install.linux.sh \
