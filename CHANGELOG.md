...
- **监控台新增「开始使用」引导卡片**：首次登录时在页面顶部把开服流程变成可点的清单——检查运行环境、创建实例、配置房间、配置世界与 Mod、启动进服、把备份交给计划任务，每一步都能直接跳到对应页面或打开对应文档；并根据你当前是否已有实例、是否有实例在运行标出"已完成 / 当前步骤 / 待办"。不需要时点「不再显示」，按账号记住，换账号登录会重新出现。**不弹强制向导**：安装教程类用户常常先四处点一圈，强制弹窗多半被直接关掉，反而什么都没记住。
