...
| 认证与会话              | ✅ 基本完成  | 登录/登出/记住密码、改密与频控；**首次登录强制改密未接入**                       |
...
| 游戏实例控制台            | ❌ 未开始   | `console` 模块仅占位，未注册路由                                  |
| 配置 / 备份 / 文件 / Mod | ❌ 未开始   | 模块占位；`backup`/`instance_mods` 表已建，无业务 API              |
| 发布验收               | ❌ 未开始   | 主流程演练与 Bug 门槛未执行                                       |


**已注册后端模块**（`server/src/app.ts`）：`auth`、`system`、`node`、`instance`。  
**仅占位未注册**：`console`、`mod`、`config`、`backup`、`file`。
...
- **首次登录强制改密闭环**：安装脚本已写环境变量，后端/前端尚未读取并拦截未改密用户。
...
- 游戏实例实时控制台日志（WebSocket / 流式输出；`registerConsoleModule` 为空实现）。
- 向游戏服务端发送命令（如 `/say`、`/save`）并回显。
- 控制台基础交互：自动滚动、清空、复制。
...
- [~] **M1（第 1-4 周）**：登录 + 节点/实例基础管理 + 系统监控台已具备；**游戏实例控制台、实例级资源指标**仍缺。
...
1. [ ] **首次登录强制改密**：读取 `FORCE_PASSWORD_CHANGE`，登录后拦截并引导改密。
2. [ ] **游戏实例控制台**：实现 `console` 模块（日志流 + 命令下发），与监控台路由/命名区分清晰。
